...
wallsFolder.add(walls.position, 'y').min(0).max(4).step(0.01).name('Y axis')
wallsFolder.close()
...
roofFolder.add(roof.position,'y').min(0).max(4).step(0.01).name('Y axis')
roofFolder.add(roof.rotation,'y').min(0).max(2).step(0.01).name('Y rotation')
roofFolder.close()
...
doorFolder.add(door.position,'z').min(1).max(3).step(0.01)
doorFolder.add(door.position,'y').min(1).max(2).step(0.01)
doorFolder.close()
house.add(door)
...
const graveMaterial = new THREE.MeshStandardMaterial({colors:'#b2bn6b1'})
...
ambientLightFolder.add(ambientLight, 'intensity').min(0).max(1).step(0.001).name('ambient intensity')
...
const tick = () =>
{
    ...
    const ghost3Angle = - elapsedTime * 0.185
    ghost3.position.x = Math.cos(ghost3Angle) * (7 + Math.sin(elapsedTime * 0.42))
    ghost3.position.z = Math.sin(ghost3Angle) * (7 + Math.sin(elapsedTime * 0.212))
